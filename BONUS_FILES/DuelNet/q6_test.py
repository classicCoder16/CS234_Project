...
class MyDQN(Linear):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            advantages = tf.contrib.layers.fully_connected(h1, num_outputs=num_actions, activation_fn=None)
            h1_p = tf.contrib.layers.fully_connected(flattened_state, num_outputs=512)
            val = tf.contrib.layers.fully_connected(h1_p, num_outputs=1, activation_fn=None)
            mean_adv = tf.reduce_mean(advantages, axis=1, keep_dims=True)
